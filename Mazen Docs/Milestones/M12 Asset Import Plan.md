...
>
> **M12-E house — split PROVEN, dressing deferred (2026-07-05):** the 2×2 house is now placed through the four `.houseCorner` Prop anchors and drawn via the asset path (`Renderer.updateAssetInstances` / `buildHouseQuarter`), so each quarter rides its tile's slice `animMat` and the building **splits and reassembles Rubik's-style** — the signature mechanic, verified with a new single-step twist control (**G** cycles normal/slow/step; **`[` `]`** scrub; see [Controls Reference](../Controls%20Reference.md)). Walls are the imported kit `Structure_Exterior_Wall_Straight`; the roof is the **procedural** hip (the kit has no 4-way hip piece). **Kit windows/door/roof were tried and reverted** — this kit's modules are small (meant to tile many-per-room) but our tiles are ~19m, so one stretched module per edge squashes tall pieces and the four roof slopes overlap into a mess. Landed on a clean plain-wall + procedural-hip + open-entrance house, **relocated to its own hedge-free court on the −Z face** (away from the crowded +Z plaza) so it has room. **Next for the house:** either keep it simple, or the proper path — **tile many small kit modules per edge at natural proportions** for real windows/trim/detail. Two design threads spun off: [Bandaged Cube Mechanic](../Bandaged%20Cube%20Mechanic.md) (bond structures so illegal twists are *refused* instead of tearing) and [Superellipsoid Cube](../Superellipsoid%20Cube.md) (inflate the cube toward a planet).
>
> **UV V-flip gotcha ✅ (2026-07-05):** ModelIO hands back UVs in the OBJ/USD **bottom-left** origin convention, but Metal samples **top-left** — so every imported diffuse was sampling *vertically mirrored*. Invisible on uniform textures (fire pit stone, bark) but obvious on the horse statue, whose localized wood-base atlas region bled onto the tail/belly-support. Fix is one line in `AssetMesh` vertex packing: `texCoord = (u, 1 - v)`. Applies to all textured imports. (A prior attempt to *recolor* the statue's wood in the atlas was a dead end — fuzzy color-matching flattened marble regions to grey patches; reverted.)
>
