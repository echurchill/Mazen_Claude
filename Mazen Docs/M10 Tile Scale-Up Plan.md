...
- **Inter-tile gap seam (decision #5).** The 0.01 `cellSpacing` gap + the floor being inset from the walls reads as a faint dark seam at tile boundaries at the low eye height. Fix: `cellSpacing` 1.01→1.0 and/or widen `floorHalfSize` to close the floor seam.
