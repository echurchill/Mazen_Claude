...
- ~~**First-person corner-cut at face crossings (found in Phase A).**~~ ✅ **Fixed (2026-07-03).** `CameraState.firstPersonCamera` lerped `eyePos` with `mix()` + renormalize — the straight chord between two points on adjacent faces passes *inside* the corner. Fix: `slerp` the eye *direction* around the shared edge (constant-angular-velocity great-circle arc) and interpolate the radius separately; the same `slerp` now drives the `up` vector so the whole camera basis pivots rigidly around the edge. Verified in first person — the eye rounds the seam smoothly.
- ~~**Floor/path texture pops on slice rotation (found in Phase E).**~~ ✅ **Fixed (2026-07-03).** The real cause was subtler than first guessed: the floor UVs are glued to the tile's *local* frame (`uv = (localXY+hs)·uvScale`). During a Q/E turn the mesh spins via `animMat` (texture rides along, fine), but at finalization the mesh snaps back to face-aligned while `openings` rotate to fix the *geometry* — nothing rotated the *UVs*, so the moss/gravel pattern jumped 90°. Walls hide it behind their height-banded look; flat floors don't. (The `styleSeed` hue and the path-cross split are actually continuous — hue travels with the facelet, and the paved stripe reorients *with* the tile.) Fix: a per-facelet `MazeTile.uvTurns` accumulator, incremented by the same `quarterTurns` that rotates `openings` in `applySliceRotation`; floor meshes are now cached per `(openings, uvTurns)` and their UVs counter-rotate by `−uvTurns·90°` (sense derived from `DirectionMask.rotated`, i.e. `<<1` = local +90°), keeping the ground texture glued to the tile through finalization. Verified: no pop on Q/E. Walls left as-is (stable by nature).
...
- ~~**Shadow acne / "tearing" (found in Phase F).**~~ ✅ **Fixed (2026-07-03), two distinct causes.** (a) *Floor/wall acne:* the PCF used `texelSize = 1/1024` after the shadow map was bumped to 2048 (double-spaced samples), and the ortho frustum was `size`…`5·size` (huge depth range → oversized world bias). Fix: `texelSize` → 1/2048; hug the frustum to the cube (`shadowNearZ` 1.8·size, `shadowFarZ` 4.2·size, `shadowOrthoRadius` 1.1·size); bias 0.0008 / 0.0025. (b) *Post hatching, worst where posts meet walls:* posts are thin markers embedded in the wall volume by design, so their surface fought the wall's depth in the shadow map. A large bias didn't help (not simple acne); confirmed shadow-related by a disable-shadows test. Fix: exclude posts (materialID 8) from *receiving* shadows (like the fog layers) — they still cast.
- ~~**Corner-post z-fighting / hedge "crack" at wall joints.**~~ ✅ **Fixed properly (2026-07-03).** A disable-posts diagnostic isolated the 45° corner ghosting to the corner posts (their faces were coplanar with the wall faces). First attempt recessed them inside the wall — but that left invisible geometry masking the underlying issue: the **hedge itself had a crack** (`tileMeshSize` 0.98 inside a 1.0 cell → a 0.02 gap between adjacent wall segments), which the corner posts existed only to hide. Proper fix (per user): set `tileMeshSize = cellSpacing = 1.0` so collinear wall segments meet edge-to-edge → **continuous hedge, no crack** (they don't z-fight: outer faces adjacent not overlapping, end caps back-to-back with one culled), and **remove the corner posts entirely** (no crack to cover, fewer polygons). Jamb posts (gateway framing) kept.
...
>
> **M10 Phase G ✅ done (2026-07-03).** Props system across five increments: **G1** data model, **G2** first sub-cell prop, **G3** landmark, **G4** interactive + hook, **G5** multi-tile house. **This completes M10.**
> - **G1 + G2 ✅.** `Prop`/`PropKind` anchored to `MazeFacelet` (so slice rotations carry props for free via the same `worldMatrix`×`animMat` path as tiles); `Prop.rotate(quarterTurns:)` spins its sub-cell + facing in the `DirectionMask.rotated` sense, keeping props glued through finalization (incremented in `applySliceRotation` alongside `openings`/`uvTurns`). Sub-cell **corner props need no collision** — the player only walks the path cross. First prop: a procedural **topiary** (squashed green ellipsoid, materialID 10 = the shader's generic lit + shadow-receiving branch, so no shader change; per-kind colour from `SceneBuilder.propColors`), placed one-per-tile in the start plaza (`stampDemoProps`). Casts + receives shadows (rides the existing opaque draw list into the shadow pass).
> - **G3 + G4 ✅.** **Obelisk** landmark (tapered shaft + pyramidion, ~1.16 tall vs 0.24 hedge after the user asked for ~2×) in the plaza-centre SE corner. **Chest** + the **interaction hook**: `GameState.interact()` (F key on macOS) toggles interactive props on the player's current tile; the chest is a box whose open/closed state shows as a colour swap (wood → gold), the dispatch point where a lever→rotation or a portal→M11 would later hang. Verified: obelisk reads over the hedges, F toggles the chest.
> - **G5 ✅.** A 2×2 **modular house**: one `houseCorner` quarter per tile (an L of two outer walls + a hip-roof slope), each occupying only its tile's inner-corner cell so the assembled building is compact and centred; `facing` (now applied to the prop transform as `facing.rawValue·45°`, also enabling asymmetric props generally) rotates each quarter's peak onto the shared 2×2 centre, where they meet with no gap. The four tiles merge into a courtyard via `stampRoom` (interior hedges removed so nothing tangles the building, perimeter gateways kept as doors — same as the plaza). Placed at the face edge so an adjacent-face slice rotation splits it Rubik's-style, each quarter riding its own tile. Iterated with the user: tile-filling → inner-corner (walkable surround), closed the seam gaps, added courtyard doors.
> - **Art direction (deferred): external 3D assets.** Real models (e.g. Poly Haven CC0 — stump / tree / fire pit) are a **separate "G-assets" pipeline**, not a mesh swap: needs a ModelIO OBJ/USD loader, a **32-bit index buffer** (imported meshes exceed the shared buffer's UInt16 ceiling), a **textured material path** in the shader (+per-model texture load), and scale/up-axis/shadow-winding normalization. **Decision (user): finish the procedural prop system first (G3–G5 as placeholders), then build the import pipeline and swap in real art.** The prop *system* is mesh-agnostic, so none of G1–G5 is wasted. → Planned as **M12 (Asset Import Plan)**, sequenced after M9; it also carries the deferred **house-split verification**.
