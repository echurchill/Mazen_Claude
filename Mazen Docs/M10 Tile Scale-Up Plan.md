...
> - **Art direction (deferred): external 3D assets.** Real models (e.g. Poly Haven CC0 — stump / tree / fire pit) are a **separate "G-assets" pipeline**, not a mesh swap: needs a ModelIO OBJ/USD loader, a **32-bit index buffer** (imported meshes exceed the shared buffer's UInt16 ceiling), a **textured material path** in the shader (+per-model texture load), and scale/up-axis/shadow-winding normalization. **Decision (user): finish the procedural prop system first (G3–G5 as placeholders), then build the import pipeline and swap in real art.** The prop *system* is mesh-agnostic, so none of G1–G5 is wasted.
