...
- ~~**Corner-post z-fighting at wall joints (the real "45° crack tearing").**~~ ✅ **Fixed (2026-07-03).** The persistent ghosting at wall corners on 45° grazing views was *not* shadow — a disable-posts diagnostic isolated it. The slim corner posts spanned exactly the wall thickness (`[hs-wt, hs]`), so their outer/inner vertical faces were **coplanar** with the wall faces → z-fighting at grazing angles. Fix: make the corner post narrower than the wall (`0.8·wt`) and center it *within* the wall thickness so its faces sit ~0.007 inside the wall faces (wall reliably wins the depth test); it pokes above the hedge as a cap so the marker still reads. (Jamb posts are wider than the wall → already proud, no coplanarity.)
