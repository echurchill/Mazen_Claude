...
- [ ] **R2.13 Force-unwrap hygiene** — `Self.faceColors[face]!` per tile per frame → `switch`/array; document the `worldStack.last!` never-empty invariant. *Confidence:* **High**. *Danger:* **Low**. *Effort:* trivial.

- [ ] **R2.14 Doc drift** — `CelestialSystem.swift:10` says "cameraFarZ = 100" (it's 220); Master Roadmap M14/M14b entries still read "uncommitted"/"planned". *Confidence:* **High**. *Danger:* **None**. *Effort:* trivial.
