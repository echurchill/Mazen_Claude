...
class GameState {
    ...
    var depthsMemo: (version: UInt64, depths: [Int: Int])? = nil   // access relaxed: setters live in GameStateSceneFive.swift
    ...
    var pulseFront: Float = -1   // access relaxed: setters live in GameStateSceneFive.swift
    /// Where the current died this cycle, by facelet id: -1 while it is still travelling.
    var pulseBrokeAt: Int = -1   // access relaxed: setters live in GameStateSceneFive.swift
    var pulseHold: Float = 0   // access relaxed: setters live in GameStateSceneFive.swift
    var pulseDepths: [Int: Int] = [:]   // access relaxed: setters live in GameStateSceneFive.swift
    ...
    var pulseBright: Float = 0   // access relaxed: setters live in GameStateSceneFive.swift
    ...
    var worldBloom: Float = 0   // access relaxed: setters live in GameStateSceneFive.swift
    var bloomElapsed: Float = -1   // access relaxed: setters live in GameStateSceneFive.swift
    var surveyorTile: (face: CubeFace, row: Int, col: Int)? = nil
    ...
    func faceletPosition(ofFaceletID id: Int) -> SIMD3<Float>? {
    ...
    /// THE ORDER IS THE CONTRACT. interact() used to be a 250-line if-chain where dispatch was
    /// first-match-wins and nothing declared the priority — which is exactly how the metal vessel
    /// came to swallow a Scene 3 plinth's press. Each branch is now a named handler; this array is
    ...
    private func handle_latch(_ ci: Int, _ fi: Int) -> Bool {
        ...
        if let lIdx = cubeModel.cubies[ci].facelets[fi].props.firstIndex(where: { $0.kind == .latch }) {
            ...
            guard cubeModel.cubies[ci].facelets[fi].props[lIdx].anim < 0.5 else { return true }   // already engaged
    ...
    /// Scene 5C: the basin fires the diagnostic pulse
    private func handle_basin(_ ci: Int, _ fi: Int) -> Bool {
    ...
    /// Scene 5: a rotator turns the slab it stands on
    private func handle_faceRotator(_ ci: Int, _ fi: Int) -> Bool {

        return false
    }

    /// Scene 4D: the vessel teaches the twist (inert scenery in Scene 1)
    private func handle_vessel(_ ci: Int, _ fi: Int) -> Bool {
        // Scene 4D — the VESSEL. Activating it makes it demonstrate the turn, and fail: the ring
        // attempts, the vessel strains, the anchors answer, and the twist becomes the player's.
        if cubeModel.cubies[ci].facelets[fi].props.contains(where: { $0.kind == .layeredVessel }) {
            // …but only where the world says its vessel is a teacher. Scene 1 is explicit that "if
    ...
    /// Scene 4: an anchor release — permanent, gated on the vessel
    private func handle_anchor(_ ci: Int, _ fi: Int) -> Bool {
        // Scene 4 — an ANCHOR. Unlike a switch this does not toggle: activating it RELEASES the bond
        // it holds, and that release is permanent ("The anchors may be released in any order. Each
        // release is permanent."). The turn stays refused until the last one is gone, because
        // canRotateSlice refuses while ANY bonded group straddles the slab.
        if let aIdx = cubeModel.cubies[ci].facelets[fi].props.firstIndex(where: { $0.kind == .anchor }) {
            guard cubeModel.cubies[ci].facelets[fi].props[aIdx].anim > 0.5 else { return true }  // already released
            // AN ANCHOR IS NOT A CONTROL UNTIL THE VESSEL HAS SPOKEN (Eddie, 2026-08-03). While it
            // waits it wears the vessel's mark, so refusing here is the same sentence the anchor is
            // already saying in pictures. It also removes the order that used to strand the player:
            // releasing every anchor first left the vessel with no lock to demonstrate against.
    ...
    /// Scene 3D: a touched obelisk refuses and points at its plinth
    private func handle_obeliskRebuff(_ ci: Int, _ fi: Int) -> Bool {
        if cubeModel.symbolPairedPlinths,
           let obIdx = cubeModel.cubies[ci].facelets[fi].props.firstIndex(where: { $0.kind == .obelisk }) {
            let symbol = cubeModel.cubies[ci].facelets[fi].props[obIdx].state
            if cubeModel.cubies[ci].facelets[fi].props[obIdx].anim <= 0 {
            ...
            return true                                  // it stays inactive either way
        }
        // Scene 3 — a plinth that controls ONE obelisk, by symbol, somewhere else in the chamber.
        // "Unlike Scene 2, activation is not reversible during normal play. Once raised, a plinth
        // remains active. The puzzle is cumulative."
    ...
    private func handle_scene3Plinth(_ ci: Int, _ fi: Int) -> Bool {
        if cubeModel.symbolPairedPlinths,
           let capIdx = cubeModel.cubies[ci].facelets[fi].props.firstIndex(where: { $0.kind == .switchCap }) {
            guard cubeModel.cubies[ci].facelets[fi].props[capIdx].alignAnim < 0.5 else { return true }  // already raised
            let symbol = cubeModel.cubies[ci].facelets[fi].props[capIdx].state
            cubeModel.cubies[ci].facelets[fi].props[capIdx].alignAnim = 1
            pendingAudioCues.append(.switchEngaged(at: nil))
            // "A pulse travels away through embedded channels… the matching obelisk activates
    ...
    /// Scene 2 / M16.6: the four-corner switches toggle
    private func handle_cornerSwitch(_ ci: Int, _ fi: Int) -> Bool {
        if let capIdx = cubeModel.cubies[ci].facelets[fi].props.firstIndex(where: { $0.kind == .switchCap }) {
            // Once the door is open (portal activated), the switches are inert for this puzzle (Eddie).
            guard templeDoorStillSealed() else { return true }
            let engaged = cubeModel.cubies[ci].facelets[fi].props[capIdx].alignAnim > 0.5
            cubeModel.cubies[ci].facelets[fi].props[capIdx].alignAnim = engaged ? 0 : 1
            // "A short tone sounds… the tone plays in reverse" on disengage (Scene 2E).
            let where_ = sliceCentre(axis: 0, index: cubeModel.cubies[ci].position.x >= 0 ? Int(cubeModel.cubies[ci].position.x) : 0)
    ...
    /// M12: a chest toggles open/shut
    private func handle_chest(_ ci: Int, _ fi: Int) -> Bool {
        for pi in cubeModel.cubies[ci].facelets[fi].props.indices
